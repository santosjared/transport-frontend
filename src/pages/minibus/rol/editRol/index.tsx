...
import { addRol, updateRol } from "src/store/apps/rol"
...
const EditRol = ({ open, toggle, data }: Props) => {
  ...
  useEffect(() => {
    if (data) {
      const selectIds = data.access.map((value:any)=>{
        return value._id
      })
      const selectNames = data.access.map((value:any)=>{
        return value.name
      })
      if(selectNames.length === 44){
        setSelectAll(true)
      }
      setSelectedPermissions(selectIds)
      setCheckOption(selectNames)
      setName(data.name)
    }
  }, [data, open, toggle]);
  ...
  const handleSubmit = async () => {
    try {
      const response = await dispatch(updateRol({data:{ name: name, access: selectedPermissions },id:data.id}))
      ...
      Swal.fire({ title: '¡Error!', text: 'ocurio un error al actualizar los datos', icon: "error" });
      handleReset()
    } finally {
      setIsLoading(false)
    }
